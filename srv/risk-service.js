...
module.exports = cds.service.impl(async function () {
    ...
    this.after("READ", Risks, (data) => {
        ...
        risks.forEach((risk) => {
            if (risk.impact >= 100000) { risk.criticality = 1; } else {
                risk.criticality = 2;
            }
            // set criticality for priority
            switch (risk.prio_code) { case 'H': risk.PrioCriticality = 1; break; case 'M': risk.PrioCriticality = 2; break; case 'L': risk.PrioCriticality = 3; break; default: break; }
        })
    })
    ...
    this.on("READ", Risks, async (req, next) => {
        ...
        } const risks = await next(); const asArray = x => Array.isArray(x) ? x : [x];
